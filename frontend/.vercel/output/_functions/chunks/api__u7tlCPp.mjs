...
const fetchWithRetry = async (url, config, maxRetries = 3, baseDelay = 1e3) => {
  let lastError;
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = err;
      const isNetworkError = err instanceof TypeError && (err.message === "Failed to fetch" || err.message.includes("network") || err.message.includes("NetworkError") || err.message.includes("Connection"));
